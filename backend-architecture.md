...
- **Soft-delete** для счетов (поле `deleted_at`), транзакции удаляются жёстко с каскадом на `transaction_shares` и `transaction_tags`.


## Общий справочник

Категории и теги доступны всем пользователям. Названия категорий уникальны в пределах типа, названия тегов — во всём справочнике, без учёта регистра. Поле `user_id` хранит создателя записи и не ограничивает доступ к ней.

Таблицы `hidden_categories` и `hidden_tags` хранят персональное скрытие. Оно влияет на отображение справочника и выбор в форме операции, но не меняет связи с транзакциями. История и аналитика используют настоящие названия и оформление записей, а фильтры позволяют выбирать скрытые записи в отдельных раскрываемых списках.

Связанные категории и теги нельзя удалить: сервис категории проверяет наличие операций, а внешние ключи защищают обе сущности от гонки с созданием транзакции. Удаление записи справочника не удаляет связи с историческими операциями; при наличии таких связей API возвращает `409 Conflict`.

Интеграционный тест: `CATALOG_TEST_DATABASE_URL=... go test ./internal/repository -run TestSharedCatalogIntegration -v`. Он использует отдельную схему PostgreSQL, проверяет применение миграций и сохранность связей, доступ двух пользователей, персональное скрытие, историю, фильтры и аналитику, затем удаляет тестовую схему.
