...
### Общий справочник (TASK-10)

Миграция 00018 объединяет совпадающие категории по типу и названию, теги — по названию, без учёта регистра; связи операций переносятся на общую запись. `user_id` хранит создателя. Таблицы `hidden_categories` и `hidden_tags` хранят персональное скрытие. Старое мягкое удаление категории переносится в скрытие у её создателя. Обратная миграция восстанавливает схему, но не разделяет объединённые записи.

Связанные категории и теги нельзя удалить: сервис категории проверяет наличие операций, а внешние ключи защищают обе сущности от гонки с созданием транзакции. Для тегов удалён каскад удаления связей.

Интеграционный тест: `CATALOG_TEST_DATABASE_URL=... go test ./internal/repository -run TestSharedCatalogIntegration -v`. Он создаёт отдельную схему, проверяет миграцию 17 → 18, доступ двух пользователей, скрытие, историю, фильтры и аналитику, затем удаляет тестовую схему.
