...
`accessMode`: `"personal"` | `"shared"`. Валюта, режим, участники и доли неизменяемы после создания.

Для совместного счёта обязателен полный массив `members`, включая владельца:

```json
{"name":"Семейный","accessMode":"shared","kind":"spending","currency":"RUB","initialBalanceDate":"2026-09-01","members":[{"username":"alice","defaultShare":0.6},{"username":"bob","defaultShare":0.4}]}
```

Пользователи должны существовать, быть активными и не повторяться. Доли от 0 до 1 имеют не более четырёх знаков после запятой и в сумме равны 1; нарушения возвращают `400`. Личный счёт без `members` автоматически получает владельца с долей 1; явно переданный состав может содержать только владельца с долей 1. Создание счёта и всех членств атомарно. Опциональное `acceptTransfers` включает приём внешних переводов; по умолчанию `false`.
...
Обновление редактируемых полей счёта. `acceptTransfers` может менять только владелец (`403` иначе); отсутствие поля сохраняет настройку. Валюта, тип средств, режим и состав не изменяются. Поля `accessMode`, `kind`, `members` и неизвестные поля в PATCH возвращают `400`.
...
Изменение состава запрещено: корректный запрос возвращает `409 Conflict`.

### `PATCH /api/accounts/{accountID}/members/{userID}`

Изменение доли запрещено: корректный запрос возвращает `409 Conflict`.

### `DELETE /api/accounts/{accountID}/members/{userID}`

Удаление участника запрещено: возвращает `409 Conflict`.

Эти запреты действуют и для владельца, и для других участников, в том числе на счетах без операций. Доступ посторонним по-прежнему запрещён (`403`). Для другого распределения создаётся новый счёт с переводом средств.
...
- Валюта подставляется из счёта. `defaultCurrency` и `defaultCurrencyAmount` сохраняют код базовой валюты и полную сумму операции в ней. Аналитика пропорционально распределяет эту сумму по сохранённым долям.
...
Баланс — только доли текущего пользователя в включённых в баланс счетах, оцениваемые по текущим курсам.

Расходы и доходы используют `ts.amount`, если валюта операции совпадает с запрошенной, иначе — `default_currency_amount * ts.amount / amount`, если сохранённая базовая валюта совпадает с запрошенной. Текущие курсы для этих операций не используются. Операции без нужной суммы не входят в денежный итог: `expensesMissingAmounts` и `incomeMissingAmounts` в summary содержат их количество. Группировки по категориям и тегам используют тот же расчёт и поле `missingAmounts` для каждой группы. Нулевая доля пользователя не делает итог неполным. Переводы и оценка остатков сохраняют отдельные правила расчёта.
...
Возвращает `200`: `{"available":true,"reasons":[]}`. Существующие счета,
категории, операции и участие в общих счетах не блокируют добавление данных.
Ограничения полной замены проверяются отдельно в предпросмотре.

### `POST /api/imports/monefy/preview`

Тело — байты `.db`, `Content-Type: application/octet-stream`. Query-параметр
`mode=empty` (добавление данных, по умолчанию; имя параметра сохранено для совместимости)
или `mode=replace`; неизвестное значение — 400.
Режим сохраняется в неизменяемом снимке. Для смены режима загрузите файл заново.
В `accounts` поле `name` содержит итоговое имя, `source_name` — исходное название.
При конфликте с собственным счётом (в том числе удалённым), счётом с участием
пользователя или другим счётом файла подбирается свободный суффикс `(1)`, `(2)` и
далее. Сравнение не учитывает регистр и пробелы по краям, длина ограничена 100
символами вместе с суффиксом. Существующие счета не изменяются и не объединяются.
Подтверждение повторно проверяет имена под блокировкой; при изменении возвращается
`409 account_names_changed`, необходимо создать новый предпросмотр.
Ответ `201`:
...
проверяются свободные имена новых счетов (empty) или неизменность старой истории (replace),
...
| 408 | `request_timeout` |
| 409 | `account_names_changed`, `replacement_changed`, `replacement_blocked`, `catalog_changed`, `preview_stale`, `import_busy`, `preview_storage_full`, `import_conflict` |
...
### `POST /api/imports/monefy/{previewID}/rates`

Настройка курсов для операций без однозначного исторического курса в базовую валюту Monefy. Возвращает новый неизменяемый предпросмотр (`201`). Доступен только автору предпросмотра.

```json
{"rates":{"TRY":"2.5"}}
```

Курс задаётся строкой: количество единиц базовой валюты за одну единицу валюты операции, положительное десятичное число до 12 цифр до и после точки. Неизвестная пара или неверный курс — `400 invalid_currency_rate`. Ручной курс применяется только к операциям без исторического курса.

Предпросмотр содержит `currency_rates`: массив объектов `currency`, `base_currency`, `rate` (строка), `source` (`current` или `manual`), `transactions` (число затронутых операций). Текущие курсы фиксируются при подготовке и не перечитываются при подтверждении или изменении других настроек. Пустой `rate` означает отсутствие текущего курса; подтверждение заблокировано до ручного ввода. Рассчитанные базовые суммы сохраняются вместе с операциями атомарно.
