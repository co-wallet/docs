...
При включённом учёте переводов для запрошенного `type` ответ дополнительно содержит группу с `categoryId: "transfers"`, `categoryName: "Переводы"` и суммой переводов. Это служебная группа, а не UUID категории. Если подходящих переводов нет, группа отсутствует.
