...
Поле `kind` принимает `spending` (текущие средства), `savings` (сбережения без процентов),
`deposit` (срочный вклад), `savings_account` (накопительный счёт с процентами,
свободным снятием и переменной ставкой), `investment` (инвестиции).
При создании без `kind` используется `spending`; неизвестное значение возвращает `400`.
После создания тип не меняется. Все типы допускают личный и совместный доступ;
балансы совместных счетов показывают долю пользователя и общий остаток отдельно.
Автоначисление процентов и история ставок не поддерживаются.
...
`savings`, `deposit`, `savings_account`, `investment`. `category_icons` необязателен и разрешён только для новых
