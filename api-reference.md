...
Счета, в которых участвует пользователь. `acceptTransfers` — приём переводов от других пользователей, по умолчанию `false`. Включение доступно только владельцу, в том числе для совместных счетов.
...
Требует авторизации. По точному логину владельца возвращает только неудалённые счета с `acceptTransfers=true`. Неизвестный логин и отсутствие открытых счетов возвращают `[]`; пустой логин — `400`.
...
- `transfer`: нужен отличный от источника `toAccountId`. Отправитель должен быть участником исходного счёта; чужое назначение должно разрешать приём переводов (`403` иначе). Удалённое назначение — `404`.
...
- `shares` задаёт доли списания. Доли зачисления сохраняются сервером по участникам назначения в момент создания. Запись перевода, обеих сторон долей и тегов атомарна.
...
Одна транзакция. Для перевода просмотр доступен участникам исходного счёта или назначения; остальные получают `403`. Ответы чтения и списка содержат `accountName`, `toAccountName`, `toCurrency`, `readOnly`. Для получателя без доступа к источнику `readOnly=true`, `recipientAmount` — его историческая доля зачисления; `shares` и `tags` пусты, категория и пересчёт отправителя скрыты.

### `PATCH /api/transactions/{transactionID}`

Частичное обновление. Разрешено только участникам исходного счёта (`403` для получателя). Тип и счета операции не меняются. При изменении суммы перевода доли зачисления масштабируются в исторических пропорциях. Выключенный приём назначения не блокирует изменение существующей операции.

### `DELETE /api/transactions/{transactionID}`

Жёсткое удаление только участником исходного счёта, с каскадом на `transaction_shares`, `transfer_shares` и `transaction_tags`. Получатель без доступа к источнику получает `403`.
