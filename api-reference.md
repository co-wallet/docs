...
При включённом учёте переводов для запрошенного `type` ответ дополнительно содержит группы по противоположному счёту с `categoryId: "transfers:<account UUID>"`. Для расходов `categoryName` имеет вид `На счёт «Название»`, для доходов — `Со счёта «Название»`. Суммы переводов одного направления и счёта объединяются. Это служебные группы, а не UUID категорий; счета с одинаковыми названиями остаются отдельными группами. Если подходящих переводов нет, группы отсутствуют.
